    ...
    <layout-main>
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <f-select v-model="filter.active" :options="activeOptions" class="col-md-2 col-sm-12"
                        >На витрине</f-select
                    >
                    <f-select v-model="filter.archive" :options="archiveOptions" class="col-md-2 col-sm-12"
                        >Архивность</f-select
                    >
                </div>
                        ...
                        <div class="additional-filter pt-3 mt-3">
                            <div class="row">
                                <f-input v-model="filter.priceFrom" type="number" class="col-lg-2 col-md-3 col-sm-12">
                                    Цена
                                    <template #prepend
                                        ><span class="input-group-text">от</span></template
                                    >
                                    <template #append
                                        ><span class="input-group-text">руб.</span></template
                                    >
                                </f-input>
                                <f-input v-model="filter.priceTo" type="number" class="col-lg-2 col-md-3 col-sm-12">
                                    &nbsp;
                                    <template #prepend
                                        ><span class="input-group-text">до</span></template
                                    >
                                    <template #append
                                        ><span class="input-group-text">руб.</span></template
                                    >
                                ...
                                <f-date v-model="filter.dateFrom" class="col-lg-2 col-md-3 col-sm-12"
                                    >Дата содания от</f-date
                                >
                                <f-date v-model="filter.dateTo" class="col-lg-2 col-md-3 col-sm-12"
                            ...
                            <div class="row">
                                ...
                                >
                                    Категория
                                    <template #help
                                        >Будут показаны товары выбранной и всех дочерних категорий</template
                                    >
                                </f-custom-search-select>
                                <f-select
                            ...
                            <div class="row">
                                ...
                                >
                                <f-select v-model="filter.badges" :options="badgeOptions" class="col-md-3 col-sm-12"
                                    >Шильдики</f-select
                                >
                            </div>
                        </div>
        ...
        <div class="d-flex justify-content-between mt-3 mb-3">
            <div class="action-bar d-flex justify-content-start align-items-center">
                <span class="d-flex flex-column align-items-center mr-4">
                    ...
                    <f-select
                        class="page-size-filter__select"
                ...
                <b-button-group class="mr-4">
                    <b-button variant="success" v-if="massEmpty(massProductsType)" @click="exportProductsByFilters"
                    ...
                    >
                        <b-dropdown-item @click="exportProductsById(massAll(massProductsType))"
                            >Экспорт выбранных</b-dropdown-item
                        >
                    </b-dropdown>
                </b-button-group>
                <dropdown
                    v-if="!massEmpty(massProductsType)"
                    :items="statusItems"
                    @select="startChangeStatus"
                    class="mr-4 order-btn"
                >
                    Сменить статус
                </dropdown>
                <button
                ...
                >
                    <fa-icon icon="copy"></fa-icon> Копировать ID офферов
                </button>
                <button
                    v-if="!massEmpty(massProductsType)"
                    ...
                    class="btn btn-outline-secondary mr-3"
                >
                    <fa-icon icon="copy"></fa-icon> Копировать ID товаров
                </button>
                <button
                    v-if="!massEmpty(massProductsType)"
                    ...
                    class="btn btn-outline-secondary mr-3"
                >
                    <fa-icon icon="copy"></fa-icon> Копировать артикулы
                </button>

                <b-dropdown variant="success" text="Назначить шильдики" v-if="!massEmpty(massProductsType)">
                    <button
                        v-if="!massEmpty(massProductsType)"
                    ...
                    >
                        <fa-icon icon="cog"></fa-icon> Добавить
                    </button>
                    <button
                        v-if="!massEmpty(massProductsType)"
                    ...
                    >
                        <fa-icon icon="trash-alt"></fa-icon> Удалить
                    </button>
                </b-dropdown>
            </div>
        </div>
        <table class="table">
            <thead>
                <tr>
                    <th>
                        <input type="checkbox" :checked="checkAll" @change="e => massCheckboxAllOnPage(e)" />
                    </th>
                    <th>ID</th>
                    <th></th>
            ...
            <tbody>
                <tr v-for="product in products">
                    ...
                    <td>
                        <span
                            class="badge"
                            :class="{ 'badge-success': product.active, 'badge-danger': !product.active }"
                        >
                    ...
                    <td>
                        <span
                            class="badge"
                            :class="{ 'badge-success': !product.archive, 'badge-danger': product.archive }"
                        >
                    ...
                    <td>
                        <span
                            class="badge"
                            :class="{
                                'badge-success': isProductionDone(product.productionStatus),
                                'badge-danger': !isProductionDone(product.productionStatus),
                            }"
                        >
                    ...
                    <td>
                        <span
                            class="badge"
                            :class="{
                                'badge-success': isApprovalDone(product.approvalStatus),
                                'badge-danger': !isApprovalDone(product.approvalStatus),
                            }"
                        >
                    ...
                    <td>
                        <span
                            v-if="'shoppilotExist' in product"
                            class="badge"
                            :class="{
                                'badge-success': product.shoppilotExist,
                                'badge-danger': !product.shoppilotExist,
                            }"
                        >
                            {{ product.shoppilotExist ? 'Да' : 'Нет' }}
                        </span>
                        <template v-else
                            >Информация временно недоступна</template
                        >
                    </td>
                </tr>
            </tbody>
        ...
        <product-badges-modal
            ...
            @save="
                () => {
                    massClear(massProductsType);
                    loadPage(iCurrentPage);
                }
            "
        />
    </layout-main>
...
export default {
    mixins: [modalMixin, mediaMixin, massSelectionMixin, validationMixin, showCaseMixin],
    components: {
        BButtonGroup,
        BButton,
        BDropdown,
    ...
    methods: {
        ...
        ...mapActions(NAMESPACE, [ACT_LOAD_PAGE, ACT_UPDATE_PRODUCTION, ACT_UPDATE_ARCHIVE, ACT_UPDATE_APPROVAL]),
        loadPage(page) {
            let cleanFilter = {};
        ...
        applyStatus() {
            ...
            switch (this.massActionType) {
                case TYPE_ARCHIVE:
                    ...
                    break;
                case TYPE_PRODUCTION:
                    promise = this[ACT_UPDATE_PRODUCTION]({
                        ids,
                        status: this.massStatus,
        ...
        copyOfferIdsToClipBoard() {
            let text = this.massAll(this.massProductsType)
                .map(x => this.iProducts.find(prod => prod.id === x).offerId)
                .join(',');
        ...
        copyArticlesToClipBoard() {
            let text = this.massAll(this.massProductsType)
                .map(x => this.iProducts.find(prod => prod.id === x).vendorCode)
                .join(',');
            clipboard.writeText(text).then();
        },
        ...
        exportProductsById(productIds) {
            Services.showLoader();
            Services.net()
                .get(this.route('products.exportByProductIds'), { product_ids: productIds })
                .then(
                    data => {
        ...
        exportProductsByFilters() {
            Services.showLoader();
            Services.net()
                .get(this.route('products.exportByFilters'), { filters: this.filter })
                .then(
                    data => {
                        let link = document.createElement('a');
                        link.href = data.path;
    ...
    computed: {
        ...
        archiveOptions() {
            return [{ value: true, text: 'В архиве' }, { value: false, text: 'Не в архиве' }];
        },
        activeOptions() {
            return [{ value: true, text: 'Да' }, { value: false, text: 'Нет' }];
        },
        pageSizeOptions() {
            return [
                { value: 10, text: 'по 10' },
        ...
        priceHiddenOptions() {
            return [{ value: false, text: 'Цены видны' }, { value: true, text: 'Цены спрятаны' }];
        },
        checkAll() {
            let BreakException = {};
            ...
            try {
                this.products.forEach(product => {
                    if (!this.massAll(this.massProductsType).includes(product.id)) throw BreakException;
                });
            } catch (e) {
