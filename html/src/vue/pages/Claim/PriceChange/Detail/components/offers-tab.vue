                    ...
                    <th>
                        <template v-if="!claim.isProcessed">
                            <button class="btn btn-primary" v-if="isWorkStatus && canUpdate(blocks.claims)"
                                    ...
                                    title="Принять все необработанные изменения">Принять все
                            </button>
                            <br>
                            <button class="btn btn-primary mt-1" v-if="isWorkStatus && canUpdate(blocks.claims)"
                                    @click="rejectPrice()"
                                    title="Отклонить все необработанные изменения">Отклонить все
                            </button>
                        </template>
                    </th>
                </tr>
                    ...
                    <td v-if="canUpdate(blocks.claims)">
                        <template v-if="!offer.status">
                            <button class="btn btn-primary" v-if="isWorkStatus"
                                    @click="changePrice('accept', offer.offerId)">Принять
                            </button>
                            <br>
                            <button class="btn btn-primary mt-1" v-if="isWorkStatus"
                                    @click="rejectPrice(offer.offerId)">Отклонить
                            </button>
                        </template>
                    </td>
                </tr>
...
export default {
    ...
    methods: {
        ...
        getGuid(offerId) {
            try {
                return this.model.productsByOffers[offerId].offer.guid
            } catch (e) {
                return null
            }
        },
