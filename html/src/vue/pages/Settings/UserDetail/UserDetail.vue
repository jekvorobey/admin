    ...
    <layout-main>
        <div class="d-flex justify-content-start align-items-start">
            <div class="d-flex flex-column">
                <shadow-card title="Основная информация"
                             :buttons="canUpdate(blocks.settings) ? {onEdit: 'pencil-alt'} : {}"
                             @onEdit="openModal('userAdd')">
                    <div>
                ...
                <shadow-card title="Роли пользователя" padding="3"
                             :buttons="canUpdate(blocks.settings) ? {onAdd: 'plus'} : {}" @onAdd="changeUserRoles()">
                    <table class="table table-sm">
                        ...
                        <tr v-for="role in roles">
                            ...
                            <td v-if="canUpdate(blocks.settings)">
                                <fa-icon @click="deleteRole(role.id)" icon="trash-alt"
        ...
        <transition name="modal" v-if="canUpdate(blocks.settings)">
            <modal :close="closeModal" v-if="isModalOpen('add-user-role')">
                ...
                <div slot="body">
                    <div v-for="userFront in frontOptions">
    ...
    computed: {
        ...
        roleOptions() {
            return Object.values(this.options.roles)
                ...
                .map(role => ({id: role.id, name: role.name, front: role.front}));
        },
        frontOptions() {
            return Object.values(this.options.fronts)
                .filter(front => this.user.fronts.includes(front.id))
                .map(front => ({id: front.id, name: front.name}));
        },
        rolesIds() {
