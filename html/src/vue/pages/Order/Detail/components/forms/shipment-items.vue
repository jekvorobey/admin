        ...
        <b-table-simple hover small caption-top responsive>
            <b-thead>
                <b-tr>
                    ...
                    <b-th>Сумма за единицу без скидки <fa-icon icon="question-circle"
                                                             v-b-popover.hover="tooltipUnitCostHelp"></fa-icon></b-th>
                    <b-th>Скидка за единицу <fa-icon icon="question-circle"
                                                   v-b-popover.hover="tooltipUnitDiscountHelp"></fa-icon></b-th>
                    <b-th>Цена за единицу со скидкой <fa-icon icon="question-circle"
                                                            v-b-popover.hover="tooltipUnitPriceHelp"></fa-icon></b-th>
                    <b-th>Сумма без скидки <fa-icon icon="question-circle"
                                                  v-b-popover.hover="tooltipCostHelp"></fa-icon></b-th>
                    <b-th>Скидка <fa-icon icon="question-circle" v-b-popover.hover="tooltipDiscountHelp"></fa-icon></b-th>
                    <b-th>Цена со скидкой <fa-icon icon="question-circle"
                                                 v-b-popover.hover="tooltipPriceHelp"></fa-icon></b-th>
            ...
            <b-tbody>
                <template v-if="shipment.nonPackedBasketItems">
                    <tr v-for="(basketItem, key) in shipment.nonPackedBasketItems">
                        ...
                        <b-td><img :src="productPhoto(basketItem.product)" class="preview" :alt="basketItem.name" v-if="basketItem.product.mainImage"></b-td>
                        ...
                        <b-td class="with-small">
                            ...
                            <small> {{basketItem.product.length}} x {{basketItem.product.width}} x {{basketItem.product.height}} мм</small>
                        </b-td>
                        ...
                        <b-td>{{ preparePrice(basketItem.qty * basketItem.discount / basketItem.qty_original) }} руб</b-td>
                    ...
                    <template v-for="(shipmentPackage, key) in shipment.packages">
                            ...
                            <b-td v-if="canEdit" class="d-lg-none">
                                <div class="float-left">
                                    <fa-icon icon="times" title="Удалить коробку" class="cursor-pointer"
                                             @click="deleteShipmentPackage(shipmentPackage.id)" v-if="!shipment.is_problem && isAssemblingStatus"></fa-icon>
                                </div>
                            </b-td>
                            <b-td :colspan="canEdit ? 10 : 9">
                                <b-row>
                                    <div class="col-sm-9">
                                        <b>Коробка #{{ key+1 }} (ID: {{shipmentPackage.id}}, {{ shipmentPackage.package.name}}, вес брутто {{shipmentPackage.weight}} г, вес пустой коробки {{shipmentPackage.wrapper_weight}} г)</b>
                                    </div>
                                    <div class="col-sm-3">
                                        <button class="btn btn-secondary" v-if="!shipment.is_problem && isAssemblingStatus && selectedBasketItemIds.length"
                                                @click="addShipmentPackageItems(shipmentPackage)">
                                            <fa-icon icon="box"></fa-icon> Добавить в коробку
                                        </button>
                                        <modal-add-shipment-package-items :model-shipment.sync="shipment" :model-order.sync="order" :shipment-package.sync="selectedShipmentPackage" :shipment-package-num="key+1" :basket-items.sync="selectedBasketItems" @onSave="onShipmentPackageItemsAdd" v-if="Object.values(selectedShipmentPackage).length > 0 && Object.values(selectedBasketItems).length > 0"/>
                                    </div>
                                </b-row>
                            </b-td>
                            <b-td v-if="canEdit">
                        ...
                        <b-tr v-for="(item, key) in shipmentPackage.items" v-bind:key="item.id">
                            ...
                            <b-td class="with-small">
                                ...
                                <small> {{item.basketItem.product.length}} x {{item.basketItem.product.width}} x {{item.basketItem.product.height}} мм</small>
                            </b-td>
                            ...
                            <b-td>{{ preparePrice(item.qty * item.basketItem.discount / item.basketItem.qty) }} руб</b-td>
                            ...
                            <b-td v-if="canEdit">
                                <div v-if="!shipment.is_problem && isAssemblingStatus" class="float-right">
                                    ...
                                    <modal-edit-shipment-package-item :model-shipment.sync="shipment" :model-order.sync="order" :shipment-package.sync="selectedShipmentPackage" :shipment-package-num="key+1" :shipment-item.sync="selectedShipmentItem" :max-qty="selectedMaxQty" @onSave="onShipmentPackageItemEdit" v-if="Object.values(selectedShipmentItem).length > 0"/>
