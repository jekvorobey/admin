        ...
        <div class="row">
            <table class="table table-hover">
                ...
                <tr v-if="filtratePromoCodes.length < 1">
                    <td colspan="9" class="text-center">Промокоды не найдены!</td>
                </tr>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            Promise.all([
                Services.net().get(
                    this.getRoute('merchant.detail.marketing.promo-codes.data', {id: this.id})
                ),
                this.promoCodePromise(),
            ]).then(data => {
                this.types = data[0].types;
                this.statuses = data[0].statuses;
                this.promoCodes = data[1].promoCodes;
                this.creators = data[1].creators;
                this.owners = data[1].owners;
            }).finally(() => {
                Services.hideLoader();
            });
        },
        methods: {
            ...
            statusPromoCodes() {
                ...
                Services.net().post(this.getRoute('promo-code.status'), {}, {
                    ...
                    this.selectPromoCodes = [];
                })
            },
            promoCodePromise() {
                return Services.net().get(
                    this.getRoute('merchant.detail.marketing.promo-codes', {id: this.id})
                )
            },
            reloadPromoCodes() {
                Services.showLoader();
