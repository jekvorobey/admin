<template>
    <b-modal :id="id" title="Создание мерчанта" hide-footer ref="modal">
        <template v-slot:default="{close}">
            ...
            <div class="row">
                <v-input v-model="$v.form.inn.$model" :error="errorInn" class="col-md-6 col-12">ИНН<span
                    class="required-red">*</span></v-input>
                <v-input v-model="$v.form.kpp.$model" :error="errorKpp" class="col-md-6 col-12">КПП</v-input>
            </div>
            <div class="mb-3">
                <v-input v-model="$v.form.legal_address.$model" :error="errorLegalAddress">Юридический адрес<span
                    class="required-red">*</span></v-input>
            </div>
            <div class="mb-3">
                <v-input v-model="$v.form.fact_address.$model" :error="errorFactAddress">Фактический адрес<span
                    class="required-red">*</span></v-input>
            </div>
            ...
            <div class="mb-3">
                <v-input v-model="$v.form.payment_account.$model" :error="errorPaymentAccount">Номер банковского
                    счета<span class="required-red">*</span></v-input>
            </div>
            ...
            <div class="mb-3">
                <v-input v-model="$v.form.bank_bik.$model" :error="errorBankBik">БИК банка<span
                    class="required-red">*</span></v-input>
            </div>
            <div class="mb-3">
                <v-input v-model="$v.form.correspondent_account.$model" :error="errorCorrespondentAccount">Номер
                    корреспондентского счета<span class="required-red">*</span></v-input>
            </div>
            <div class="mb-3">
                <v-input v-model="$v.form.bank_address.$model" :error="errorBankAddress">Юридический адрес банка<span
                    class="required-red">*</span></v-input>
            </div>
            ...
            <div class="row">
                <v-input v-model="$v.form.last_name.$model" :error="errorLastName" class="col-md-4 col-12">Фамилия
                </v-input>
                <v-input v-model="$v.form.first_name.$model" :error="errorFirstName" class="col-md-4 col-12">Имя
                </v-input>
                <v-input v-model="$v.form.middle_name.$model" :error="errorMiddleName" class="col-md-4 col-12">
                    Отчество
                </v-input>
            </div>
            <div class="row">
            ...
            <div class="mb-3">
                <v-input v-model="$v.form.storage_address.$model" :error="errorStorageAddress" tag="textarea">Адреса
                    складов отгрузки<span class="required-red">*</span></v-input>
            </div>
            <div class="mb-3">
                <v-input v-model="$v.form.site.$model" :error="errorSite">Сайт компании<span
                    class="required-red">*</span></v-input>
            </div>
            <div class="mb-3">
                <v-select
                    ...
                    :multiple="true"
                >Способы оплаты, недоступные для мерчанта
                </v-select>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-3">
                <v-input v-model="$v.form.sale_info.$model" :error="errorSaleInfo" tag="textarea">Бренды и товарные
                    категории, которыми вы торгуете<span class="required-red">*</span></v-input>
            </div>
