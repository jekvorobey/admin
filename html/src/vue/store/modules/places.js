...
export default {
    ...
    actions: {
        [ACT_LOAD_PAGE]({commit, rootGetters}, {page}) {
            return Services.net().get(rootGetters.getRoute('public-event.places.page'), {page})
                .then(data => {
                    commit(SET_PAGE, {
                        list: data.places,
                        total: data.total,
        ...
        [ACT_SAVE_PLACES]({rootGetters}, {id, place}) {
            return Services.net().post(rootGetters.getRoute('public-event.places.save'), {}, {id, place});
        },
